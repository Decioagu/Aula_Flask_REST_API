...
from dados_hoteis import hoteis # Arquivo Python (lista de hotes)
...
class Hotel(Resource):
    ...
    # Função auxiliar (Filtro por id)
    def find_hotel(hotel_id):
        for hotel in hoteis: # (lista de hotes)
            if hotel['hotel_id'] == hotel_id: # (Filtro por id)
                return hotel
        return False
    ...
    def get(self, hotel_id): 
        ...
        hotel = Hotel.find_hotel(hotel_id) # Função auxiliar (Filtro por id)

        if hotel:
            return hotel, 200 # (lista de hotes)
        else:
    ...
    def post(self, hotel_id):    
        ...
        novo_hotel_objeto = HotelModel(hotel_id, **dados) # filtro por id

        # novo_hotel_dicionario = (ESCOPO Flask).(método json)
        novo_hotel_dicionario = novo_hotel_objeto.json() # transforma dados filtrado por id para JSON
        ...
        if hotel:
            ...
            return novo_hotel_dicionario, 200
    ...
    def put(self, hotel_id):
        ...
        # Se ID existir
        if hotel:
            hotel.update(novo_hotel_dicionario) # Atualizar lista (dados hoteis)
            return hotel, 200
        else:
